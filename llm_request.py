...
def make_prompt(user_message, objects, player, water_tiles, water_drops):
    ...
    Fruits have 3 stages of growth: petite, moyenne and mure as fully grown. One water drop makes a fruit grow by one stage.
    Your goal is to create a fruit salad, which is composed of 3 different mure fruits fully grown placed next to each other in a row.
    ...
    4/ "DROP_WATER" : Drop water at the current position. The player has a limited number of water drops (max 3). Dropping water on a fruit will make it grow faster. Player can have water drops when passing on water tiles.
    4/ It is IMPOSSIBLE to place multiple objects at the same place. You can't drop an object where one is already placed. You con't drop an object on water tiles
    ...
    The position of a fruit salade is defined by the position of the first object in the row. The fruits have to be fully grown to be considered as a fruit salad.
