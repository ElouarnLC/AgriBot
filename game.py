...
class CookoBot(arcade.Window):
    def __init__(self):
        ...
        self.water_tiles = set()  # Set to store water tile positions
        self.water_drops = 0  # Counter for water drops in inventory

        # Create a horizontale BoxGroup to align buttons
        self.action_box = arcade.gui.UIBoxLayout(
            vertical=False, x=MENU_X, y=INVENTORY_BUTTON_Y
        )
        self.chat_box = arcade.gui.UIBoxLayout(
        ...
        send_button = arcade.gui.UIFlatButton(
            text="Envoyer", width=BUTTON_WIDTH, style=BUTTON_MAUVE
        )
        send_button.on_click = self.send_instruction
        self.chat_box.add(
            send_button.with_space_around(top=PADDING + INSTRUCTION_BOX_PADDING)
        )

        # Créer un checkbox pour activer le LLM
        self.llm_checkbox = arcade.gui.UIFlatButton(
            text="Désactiver le LLM",
    ...
    def on_draw(self):
        ...
        if self.path and self.path_index < len(self.path):
            for step in self.path[self.path_index :]:
                ...
                arcade.draw_rectangle_filled(
                    position_x, position_y, TILE_SIZE, TILE_SIZE, (220, 205, 205)
                )
        ...
        # Afficher les gouttes d'eau
        arcade.draw_texture_rectangle(
            POINT_BOX_X,
            POINT_BOX_Y,
            LOGO_SIZE,
            LOGO_SIZE,
            arcade.load_texture("images/water.png"),
        )
        arcade.draw_text(
            f"{self.water_drops}",
            POINT_BOX_X + LOGO_SIZE - 10,
            POINT_BOX_Y - LOGO_SIZE // 2 + 10,
            arcade.color.MAUVE_TAUPE,
            16,
        ...
        arcade.draw_texture_rectangle(
            POINT_BOX_X + MENU_WIDTH // 2,
            POINT_BOX_Y,
            LOGO_SIZE,
            LOGO_SIZE,
            arcade.load_texture("images/star.png"),
        )
        arcade.draw_text(
            f"{self.action_count}",
            POINT_BOX_X + MENU_WIDTH // 2 + LOGO_SIZE - 10,
            POINT_BOX_Y - LOGO_SIZE // 2 + 10,
            arcade.color.MAUVE_TAUPE,
            16,
            font_name="Comic Sans MS",
        ...
        arcade.draw_text(
            f"Inventaire ({len(self.inventory)}/{INVENTORY_SIZE})",
            MENU_X,
            INVENTORY_TITLE_Y,
            arcade.color.MAUVE_TAUPE,
            16,
        ...
        arcade.draw_rectangle_outline(
            MENU_X + MENU_WIDTH // 2,
            INVENTORY_BOX_Y - INVENTORY_BOX_HEIGHT // 2,
            MENU_WIDTH,
        ...
        arcade.draw_rectangle_outline(
            MENU_X + MENU_WIDTH // 2,
            INSTRUCTION_BOX_Y - INSTRUCTION_BOX_HEIGHT // 2,
            MENU_WIDTH,
    ...
    def on_activation_llm_press(self, event=None):
        ...
        self.llm_checkbox.text = (
            "Désactiver le LLM" if self.llm_activated else "Activer le LLM"
        )
    ...
    def a_star(self, start_x, start_y, goal_x, goal_y):
        ...
        g_score = {
            pos: float("inf")
            for pos in [(x, y) for x in range(NB_TILES) for y in range(NB_TILES)]
        }
        ...
        while not open_set.empty():
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:  # Déplacement N/S/E/O
                neighbor = (x + dx, y + dy)
                if (
                    0 <= neighbor[0] < NB_TILES and 0 <= neighbor[1] < NB_TILES
    ...
    def move_along_path(self, delta_time):
        ...
        if self.path_index < len(self.path):
            ...
            if (self.player["x"], self.player["y"]) in self.water_tiles:
                if self.water_drops < 3:
                    self.water_drops += 1
            self.on_draw()  # Redessine l'écran pour montrer le mouvement
        else:
            arcade.unschedule(
                self.move_along_path
            )  # Arrête la planification lorsque le déplacement est terminé
            self.execute_stack()
    ...
    def action_move(self, event=None):
        ...
        arcade.schedule(
            self.move_along_path, MOVE_DELAY
        )  # Planifie la fonction de déplacement
    ...
    def action_pick(self, event=None):
        ...
        if current_pos in self.items_on_map:
            ...
            if len(self.inventory) == INVENTORY_SIZE:  # Si l'inventaire est plein
                ...
                )  # Retirer l'objet le plus ancien
                self.items_on_map[current_pos] = (
                    dropped_item  # Le déposer sur la cellule
                )
            else:
    ...
    def action_drop(self, event=None):
        ...
        current_pos = (self.player["x"], self.player["y"])
        self.action_count += 1  # Incrémente le compteur d'actions
        if current_pos in self.water_tiles:
            print("Impossible de déposer un objet sur une tuile d'eau")
            return None
        if current_pos not in self.items_on_map:
            # Déposer l'objet sur la carte
            item_to_drop = (
                self.inventory.popleft()
            )  # Retirer le dernier objet de l'inventaire
